/***************************************************
 * Release Note NB : 
 * V1.1 : enlever le Random si plusieurs MP3 dans le meme folder. PAr defaut on prend toujours le 001.MP3 de chaque folder.
 * V1.2 : changement de Range des numeros, on passe à une tranche entre 1000 et 9999
 * V1.3 : réglage du volume en utilisant la plage 9992 à 9997
 * 
...
const unsigned int gc_minYearToCall = 1000; // older year available in SD card
const unsigned int gc_maxYearToCall = 9999; // earlier year available in SD card
const unsigned int gc_00YearFolderReplacementNumber = 99; // ugly trick ! folder number replacement, used for year 00 (like 2000, since folder "00" is not supported by chip  :-/)
...
{    
  ...
  {    
    // NB:On ne conserve que les 2 derniers digits
    p_numberDialed = p_numberDialed.substring(p_numberDialed.length() - 2);
    l_yearToCall = p_numberDialed.toInt();
    
    /*
     * NB : On ne gère pas les années dans notre cas, on fait ne garde que les 2 derniers digits. 
    ...
    */
    ...
    // ugly trick : MP3 player cannot read into "00" folder =>> let's read special folder number for 00 ... :-/
    if (l_yearToCall == 0)
    {
      l_yearToCall = gc_00YearFolderReplacementNumber;
    }

    // Réglage du Volume en utilisant la plage 92 à 97
    if (l_yearToCall == 92)
    {
      gc_MP3Volume = 5;
      Serial.println("volume à " + gc_MP3Volume);
    }
...
* NB : modif du code pour forcer à 1 fichier uniquement par folder
*/
...
void callAndPlaySong (unsigned short p_folderToCall, unsigned short p_fileToPlay)
{
  ...
  Serial.println("volume à " + string (c_MP3Volume)); 
