...
SoftwareSerial mySoftwareSerial(D9, D10); // RX, TX
...
const unsigned int gc_hangUpPin = D8;
// WARNING ! 
...
const unsigned int gc_dialPin = D7; // WARNING ! Interruption pin : rotaring pulses (numbers) are caught here
const unsigned int gc_rotaringPin =D6; // pin used to know if we are currently rotaring.
...
bool isPhoneHangedUp()
{
  ...
  if (sensorVal == HIGH) {
    ...
    Serial.println("Tel deccrocher ...");   
  } else {
    digitalWrite(LED_BUILTIN, HIGH);
    Serial.println("tel raccroché ...");
  }
